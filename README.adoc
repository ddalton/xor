...
1. UI model - For e.g., schema section in swagger for REST APIs
2. Business logic model - For e.g., Business logic classes modelled in an ORM 
3. Database model - For e.g., tables and foreign keys in an RDBMS
...
= Use cases
In a J2EE environment, a couple use case patterns rise to the top in terms of performance related to persistence.
...
== Data generation
There are many cases when it is necessary to have a large data set against a J2EE application. Some of the cases are:
...
As you can see in both the above use cases, performance concerns are addressed by reducing the number of transformations that data needs to go through. 
XOR allows the user to work with the data model closest to the task at hand.
We improved the performance for the Data retrieval use case by working with the UI model and for the Data generation use case by working with the JDBC model.
