...
XOR is a tool that allows users to perform CRUDQ (Create, Read, Update, Delete, Query) operations against a Data Model using a uniform interface. It has mechanisms to allow a user to control how some of these operations are implemented. This allows it to address performance concerns related to persistence in a J2EE environment.
