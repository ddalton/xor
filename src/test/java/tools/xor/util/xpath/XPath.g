...
locationPath 
  ...
  |  absolutePathI relativeLocationPath
  ;
...
step  :  axisSpecifierI nodeTestI predicateI*
  |  abbreviatedStepI
  ;

axisSpecifierI
  :  axisSpecifier -> ^(AXIS axisSpecifier?)
  ;
...
abbreviatedStepI
  :  abbreviatedStep -> ^(ABBRSTEP abbreviatedStep)
  ;
...
unionExprNoRoot
  :  pathExprNoRootI ('|' unionExprNoRoot)? 
  |  '/' '|' unionExprNoRoot
  ;

pathExprNoRootI
  :  pathExprNoRoot -> ^(EXPR pathExprNoRoot)
  ;
